...
000002bc <get_siblings_info>:
SYSCALL(get_siblings_info)
 2bc:	b8 16 00 00 00       	mov    $0x16,%eax
 2c1:	cd 40                	int    $0x40
 2c3:	c3                   	ret    

000002c4 <get_ancestors>:
...
free(void *ap)
{
 ...
 4ef:	eb 02                	jmp    4f3 <free+0x19>
 4f1:	89 d0                	mov    %edx,%eax
 4f3:	39 c8                	cmp    %ecx,%eax
 4f5:	73 04                	jae    4fb <free+0x21>
 4f7:	39 08                	cmp    %ecx,(%eax)
 4f9:	77 12                	ja     50d <free+0x33>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
 4fb:	8b 10                	mov    (%eax),%edx
 4fd:	39 c2                	cmp    %eax,%edx
 4ff:	77 f0                	ja     4f1 <free+0x17>
 501:	39 c8                	cmp    %ecx,%eax
 503:	72 08                	jb     50d <free+0x33>
 505:	39 ca                	cmp    %ecx,%edx
 507:	77 04                	ja     50d <free+0x33>
 509:	89 d0                	mov    %edx,%eax
 50b:	eb e6                	jmp    4f3 <free+0x19>
      break;
...
morecore(uint nu)
{
 ...
 56d:	50                   	push   %eax
 56e:	e8 31 fd ff ff       	call   2a4 <sbrk>
  if(p == (char*)-1)
 ...
 584:	50                   	push   %eax
 585:	e8 50 ff ff ff       	call   4da <free>
  return freep;
...
malloc(uint nbytes)
{
 ...
 5bd:	74 04                	je     5c3 <malloc+0x25>
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 5bf:	8b 01                	mov    (%ecx),%eax
 5c1:	eb 4b                	jmp    60e <malloc+0x70>
    base.s.ptr = freep = prevp = &base;
 ...
 5e6:	eb d7                	jmp    5bf <malloc+0x21>
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
 5e8:	74 1a                	je     604 <malloc+0x66>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 ...
 613:	73 d3                	jae    5e8 <malloc+0x4a>
    if(p == freep)
 615:	39 05 f4 08 00 00    	cmp    %eax,0x8f4
 61b:	75 ed                	jne    60a <malloc+0x6c>
      if((p = morecore(nunits)) == 0)
 61d:	89 d8                	mov    %ebx,%eax
 61f:	e8 2a ff ff ff       	call   54e <morecore>
 624:	85 c0                	test   %eax,%eax
 626:	75 e2                	jne    60a <malloc+0x6c>
 628:	eb d4                	jmp    5fe <malloc+0x60>
